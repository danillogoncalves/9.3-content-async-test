...
describe('Teste a função fetchCharacter', () => {
  it('Verifique se o nome da personagem é Wonder Woman', async () => {
    const character = await fetchCharacter('720');
    expect(character.name).toBe('Wonder Woman');
  });
  it('Verifique se retorna erro ao executar a função sem parâmetro', async () => {
    const failRequest = await fetchCharacter();
    expect(failRequest).toEqual(new Error('You must provide an url'))
  });
  it(`Verifica se retorna \'Invalid is'\ ao executar a função com parâmetro que não existe`, async () => {
